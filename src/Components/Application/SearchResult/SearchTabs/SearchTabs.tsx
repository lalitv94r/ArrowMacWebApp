...
import { SearchTabsPropsType } from "@/Type/Application/SearchResult/SearchResult";
      ...
      <Nav tabs className="search-list">
        {navData.map((data, index) => (
          <NavItem key={index} className={data.color !== "" ? `bg-light-${data.color}` : ""}>
            <NavLink className={`${activeTab === index + 1 ? "active" : ""} ${data.color !== "" ? `txt-${data.color}` : ""}`} onClick={() => handleTab(data.id)} href={Href}>
