...
import { ComposeMessage, EnterYourMessages, FromHeading, Href, SaveAsDraft, Send, Subject } from "@/Constant";
import SimpleMdeReact from "react-simplemde-editor";
import { useState } from "react";
import { SubmitHandler, useForm } from "react-hook-form";
import { AddNewEmailInterFace } from "@/Type/Application/LetterBox/LetterBox";
    ...
    <Modal isOpen={composeEmail} toggle={() => dispatch(setComposeEmail(false))} size="lg" id="compose_mail">
      ...
      <ModalBody className="compose-modal">
        <Form className="needs-validation" onSubmit={handleSubmit(onSubmit)}>
              ...
              <Col sm="10">
                ...
                <div className="add-bcc">
                  <div className="d-flex gap-2">
                    <Link className="btn" href={Href} onClick={() => setCcShow(!ccShow)}>
                      {" "}
                      Cc
                    </Link>
                    <Link className="btn" href={Href} onClick={() => setBccShow(!bccShow)}>
                      {" "}
                      Bcc{" "}
                    </Link>
                  </div>
          ...
          <ModalFooter>
            <Button color="primary" className="btn-light-primary" onClick={() => dispatch(setComposeEmail(false))}>
              {SaveAsDraft}
            </Button>
