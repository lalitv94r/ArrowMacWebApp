...
import { AddNewRowButton, AddRowsTitle, SearchTableButton } from '@/Constant';
...
const AddRows = () => {
    ...
    return (
      <Col sm="12">
        <Card>
          <CardHeader className='state-ellipsis'>
            <h4 className="mb-3">{AddRowsTitle} </h4>
            <span>New rows can be added to a DataTable using the<code className="api" title="DataTables API method">row.add()</code>API method. Simply call the API function with the data for the new row (be it an array or object). Multiple rows can be added using the<code className="api" title="DataTables API method">rows.add()</code>method (note the plural). Data can likewise be updated with the<code className="api" title="DataTables API method">row().data()</code>and<code className="api" title="DataTables API method">row().remove()</code>methods.</span><span>Note that in order to see the new row in the table you must call the<code className="api" title="DataTables API method">draw()</code>method, which is easily done through the chaining that the DataTables API employs.</span>
          </CardHeader>
          <CardBody>
            <Button color="primary" className="mb-3" onClick={addNewRow}>{AddNewRowButton}</Button>
            <div className="table-responsive ">
              <div id="API-1" className="dataTables_wrapper"><DataTable data={filteredItems} columns={addRowColumn} striped={true} pagination className="display custom-scrollbar" subHeader subHeaderComponent={subHeaderComponentMemo} /></div>
            </div>
          </CardBody>
        </Card>
      </Col>
