import { BrandName, BrandNamePlaceholder, BrandSlug, BrandSlugPlaceholder } from '@/Constant';
import { BrandInitialValues, BrandValidation } from '@/Data/Forms/FormsControl/Brands/Brands';
import useFetch from '@/network';
import api_urls from '@/network/apiUrls';
import { BrandFormProps, BrandValidationProp } from '@/Type/Forms/FormControls/FormsControls';
                ...
                {({ values, handleChange, submitForm, resetForm, errors, setFieldValue, setFormikState }) => {
                    useEffect(() => {
                        const fetchBrandById = async () => {
                            try {
                                let result: any = await get(`${api_urls?.brands}${params?.params?.[0]}/`);
