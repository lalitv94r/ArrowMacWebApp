...
import { AddNewBrand, BrandEntryHeading, BrandsHeading } from '@/Constant';
...
const BrandEntry = () => {
    ...
    const handleSubmitBrand = async (values: BrandValidationProp, { resetForm }: { resetForm: () => void }) => {
        ...
        if (!isNotNull(params?.params?.[0])) {
            // Create new brand
            try {
                let response: any = await post(api_urls?.brands, body);
            ...
            // Update brand
            try {
                let response: any = await put(`${api_urls?.brands}${params?.params?.[0]}/`, body);
                console.log("RESPONSE", response);
            ...
            <Container fluid className="brand-entry">
                <Col md="6" lg="4">
                    <Card>
                        ...
                        <CardBody>
                            <BrandForm handleSubmitBrand={handleSubmitBrand} />
                        </CardBody>
                    </Card>
                </Col>
            </Container>
