...
import { BrandListHeading, BrandsHeading } from '@/Constant';
...
const Brands: React.FC = () => {
    ...
    useEffect(() => {
        const fetchBrandsList = async () => {
            try {
                const result = await get(`${api_urls?.brands}?page=${currentPage}&search=${search}`);
    ...
    const deleteItem = async (item: any) => {
        try {
            let result: any = await del(`${api_urls?.brands}${item?.id}/`);
            if (result?.status === 200) {
    ...
    return (
        <div>
            ...
            <Breadcrumbs mainTitle={BrandListHeading} parent={BrandsHeading} />
                        ...
                        brands?.results?.length > 0 ?
                            <Fragment>
                                <Table hover size={"md"} responsive>
                                    ...
                                    <tbody>
                                        {
                                            brands?.results?.map((i: any, idx: React.Key) => (
                                                <tr key={i?.id}>
