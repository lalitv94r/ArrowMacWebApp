...
import { ScrollImageType, ScrollVerticalType, StateSavingTableDataType, TableActionTypes, ZeroConfigurationTableColumnsType, CustomCellInterFaces } from "@/Type/Tables/DataTables/DataTables";
...
const TableAction = ({ id }: TableActionTypes) => {
  return (
...
const CustomBadge: React.FC<CustomCellInterFaces> = ({ position, color }) => {
  return (
    <Badge pill className={`badge-light-${color}`} color="">
      {position}
    </Badge>
  );
};
...
const ScrollImage: React.FC<ScrollImageType> = ({ image, title }) => {
  return (
    <>
      <Image width={33} height={32} className="img-fluid table-avtar" src={`${ImagePath}/${image}`} alt="userImage" /> {title}
    </>
  );
};
